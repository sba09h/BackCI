...
export const sign = async (req, res) => {
  ...
  try {
    ...
    if (run != userFound.run)
      return res.status(400).json({ message: "run no encontrado" });
    const contratoFound = await Contract.findById({ _id: idContrato });
    if (!contratoFound)
      return res.status(400).json({ message: "id not found" });

    //asociar contrato a usuario
    userFound.docFirmados.push(contratoFound._id, contratoFound.date.now ); 
...
export const contratosFirmados = async (req, res) => {
  ...
  try {
    ...
    if (!user)
      return res
        .status(400)
        .json({ message: "Error en usuario y/o contrato no encontrados" });
...
export const mandarCorreo = async (user, contrato) => {
  const html = `<h1>CONTRATO FIRMADO</h1>
    <p>Estimado(a): ${user.nombre}, te adjuntamos a continuación el consentimiento informado que acabas de firmar con nosotros</p>
      <ul>
        <strong>${contrato.titulo}</strong>: ${contrato.contenido} 
        <br/>
        <strong>Número de documento:</strong> ${contrato._id}
    </ul>`;
  await sendEmail(user.correo, "Documento Firmado", html);
};
